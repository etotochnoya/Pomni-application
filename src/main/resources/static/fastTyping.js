    ...
    document.onkeypress = function (event) {
      ...
      if (text[index] === event.key) {
        ...
        if (alreadyInputDiv.textContent.length > MAX_SHOW_TEXT_SIZE) {
          alreadyInputDiv.textContent = alreadyInputDiv.textContent.substring(
            alreadyInputDiv.textContent.length - MAX_SHOW_TEXT_SIZE,
            alreadyInputDiv.textContent.length
          );
        }

        inputDiv.textContent = text.substring(
          index,
          MAX_SHOW_TEXT_SIZE + index
        );
